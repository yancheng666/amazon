...
<body>
	...
	<div class="container">
					...
					<tbody>
						...
						<tr class="active">
							<td width="60" width="40%"><input type="hidden" name="id"
								...
								height="60"></td>
							<td width="30%"><a href="${pageContext.request.contextPath}/productServlet?method=FindProductDesc&pid=${item.product.pid}">${item.product.pname}</a></td>
		...
		<div>
			<hr />
					<form class="form-horizontal"  action="${pageContext.request.contextPath }/amazonOrderServlet"
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<input type="text" class="form-control" id="username" name="address"
							placeholder="请输入收货地址" 
							<c:if test="${ not empty order.user}">value="${order.address}"</c:if>
							>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-5">
						<input type="text" class="form-control" id="inputPassword3" name="name"
							placeholder="请输收货人" 
							<c:if test="${ not empty order.user}">value="${order.user.name }"</c:if>
							>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-5">
						<input type="text" class="form-control" id="confirmpwd" name="telephone"
							placeholder="请输入联系方式"
							<c:if test="${ not empty order.user}">value="${order.user.telephone}"</c:if>
							>
					</div>
				</div>
